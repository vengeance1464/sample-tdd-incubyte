...
describe("Add function tests Success", () => {
  ...
  beforeAll(() => {
    ...
    jest
      .spyOn(StringValidationService.prototype, "validateDelimeters")
      .mockImplementation(() => {
        return true;
      });
  });
...
describe("Add function failure", () => {
  ...
  test("add() should fail for negative input", () => {
    ...
    jest
      .spyOn(StringValidationService.prototype, "validateNumbersInInput")
      .mockImplementation(() => {
        throw new Error("negative numbers not allowed -4,-7,-9");
      });
    expect(addInstance.add("-4,5\n//;\n1;2\n-7;8\n//.\n1.-9")).toBe(
      "negative numbers not allowed -4,-7,-9"
    );
  });
});
